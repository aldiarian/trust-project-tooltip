(()=>{
        // variables
        let margenTooltip = 10;
        let mq = window.matchMedia( "(max-width: 768px)" );
        let withViewport = Math.max(document.documentElement.clientWidth, window.innerWidth || 0);
        ...
        function outTooltip(evnt) {
            ...
            if ( evnt.target.classList.contains("info-trust__button") || evnt.target.tagName === "rect" || evnt.target.tagName === "path") {
                ...
                switch (tooltip.dataset.position) {
                    case "bottom":
                        if (mq.matches && infoButtonPosition.x < tooltip.offsetWidth/2 ) {
                            tooltip.classList.add('ue-c-tooltip-trust--nopointer')
                            tooltip.style.left  = (withViewport - tooltip.offsetWidth )/2  + "px";
                            tooltip.style.top  = (infoButtonPosition.y + infoButtonPosition.height + margenTooltip + "px" );
                        }else{
                            //posiciono el mensaje debajo del botón
                            tooltip.style.left  = infoButtonPosition.x + (infoButton.offsetWidth - tooltip.offsetWidth)/2  + "px";
                            tooltip.style.top  = (infoButtonPosition.y + infoButtonPosition.height + margenTooltip + "px" );
                        }
                        // //posiciono el mensaje debajo del botón
                        // tooltip.style.left  = infoButtonPosition.x + (infoButton.offsetWidth - tooltip.offsetWidth)/2  + "px";
                    ...
                    case "top":
                        ...
                        tooltip.style.left  = infoButtonPosition.x + (infoButton.offsetWidth - tooltip.offsetWidth)/2  + "px";
                        tooltip.style.top  = ((infoButtonPosition.y - tooltip.offsetHeight) - margenTooltip + "px" );
                        break;
                    case "left":
                        //posiciono el mensaje a la drch del botón
                        tooltip.style.left  =( infoButtonPosition.x - tooltip.offsetWidth) - margenTooltip + "px";
                    ...
                    case "right":
                        ...
                        tooltip.style.left  =  infoButtonPosition.x + infoButtonPosition.width  + margenTooltip + "px";
                        tooltip.style.top  = ((infoButtonPosition.y -( tooltip.offsetHeight /2) ) + infoButtonPosition.height/2  + "px" );
                        break;
                    default:
                        break;
