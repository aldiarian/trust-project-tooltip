(()=>{
    ...
    // variables
    let tooltip = document.getElementById('ue-c_tooltip'),
        mainBox = document.querySelector('.ue-c-tooltip__main'),
        infoButton = document.getElementById('ue-c_tooltip-button¶'),
        box = document.querySelector('.ue-c-tooltip__box'),
    ...
    function outTooltip(evnt) {
        ...
        if(toolPosition.right > ( window.innerWidth / 2 ) && (mq.matches) ){
          ...
          if ( evnt.target.classList.contains("ue-c-tooltip__button") || evnt.target.tagName === "rect" || evnt.target.tagName === "path") {
